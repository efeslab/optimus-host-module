...
static int hssi_set_mode(struct pac_hssi *hssi, unsigned char mode)
{
	...
	return hssi_local_write(hssi, NIOS_LBUS_PRMGMT_RAME_ENA,
				NIOS_MAC_RX_ALIGNMENT_EN);
}

static int set_hssi_mode_10gb(struct pac_hssi *hssi)
{
	int err = 0;
...
static int set_hssi_mode_40gb(struct pac_hssi *hssi)
{
	int err = 0;
			    ...
			    const char *buf, size_t count)
{
	...
	if (hssi_eth_speed == HSSI_ETH_SPEED_10G)
		err = set_hssi_mode_10gb(hssi);
	else if (hssi_eth_speed == HSSI_ETH_SPEED_40G)
		err = set_hssi_mode_40gb(hssi);
	else
		err = -EINVAL;
...
static struct attribute_group hssi_mgmt_attr_group = {
	.name	= "hssi_mgmt",
...
static int intel_pac_hssi_probe(struct platform_device *pdev)
{
	...
	return sysfs_create_group(&pdev->dev.kobj, &hssi_mgmt_attr_group);
}
